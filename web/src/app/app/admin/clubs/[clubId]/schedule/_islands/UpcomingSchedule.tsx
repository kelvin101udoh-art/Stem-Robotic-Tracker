...
function opsQualityForDay(rows: Array<{ status?: string | null; participants?: number | null; evidence_items?: number | null; activities_total?: number | null }>) {
  // UI-only score: planned/open mix and whether core signals exist
  if (!rows.length) return { label: "NO PLAN", cls: "border-slate-200/80 bg-white/70 text-slate-700", score: 0 };
  ...
  const score = Math.max(
    ...
    Math.min(
      1,
      0.35 * (open / total) + 0.25 * (withEvidence / total) + 0.25 * (withChecklist / total) + 0.15 * (withPeople / total)
    )
  );

  if (score >= 0.75) return { label: "OPS READY", cls: "border-emerald-200/80 bg-emerald-50/70 text-emerald-950", score };
  if (score >= 0.45) return { label: "PARTIAL", cls: "border-indigo-200/80 bg-indigo-50/70 text-indigo-950", score };
  return { label: "LOW SIGNAL", cls: "border-rose-200/80 bg-rose-50/70 text-rose-950", score };
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white/60 p-4">
      <div className="flex items-center justify-between gap-2">
        <div className="text-xs font-semibold tracking-widest text-slate-500">LIVE PLACEHOLDERS</div>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="rounded-2xl border border-slate-200/80 bg-slate-50/60 p-3">
    ...
    <div className="rounded-[28px] border border-slate-200/70 bg-white/55 shadow-[0_24px_80px_-56px_rgba(2,6,23,0.45)] backdrop-blur-xl overflow-hidden">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-xs text-slate-600">
              Next 7 days • Grouped by day • Week separators • Inline ops actions (UI-only)
            </div>
          </div>
              ...
              return (
                <div key={key} className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={cx("rounded-full border px-2.5 py-1 text-[11px] font-semibold", quality.cls)}>
                        {quality.label}
                      </span>
                  ...
                  <div className="rounded-2xl border border-slate-200/80 bg-white/60 overflow-hidden">
                    {list.length ? (
                        ...
                        {list.map((s) => (
                          <div key={s.id} className="px-4 py-3 sm:px-5">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className={cx("rounded-full border px-2.5 py-1 text-[11px] font-semibold", statusChip(s.status))}>
                                ...
                                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                                  ...
                                  <span className="rounded-full border border-slate-200 bg-white/70 px-3 py-1 font-semibold text-slate-700">
                                    Evidence: <span className="text-slate-900">{s.evidence_items ?? 0}</span>
                                  </span>
                                  <span className="rounded-full border border-slate-200 bg-white/70 px-3 py-1 font-semibold text-slate-700">
                                    Checklist: <span className="text-slate-900">{s.activities_total ?? 0}</span>
                                  </span>
                                </div>
                              </div>

                              {/* Inline actions (UI-only) */}
                              <div className="flex flex-wrap items-center gap-2">
                                <button
                                  onClick={() => openDrawer("open", s.id)}
                                  className="rounded-xl border border-emerald-200/80 bg-emerald-50/70 px-3 py-2 text-xs font-semibold text-emerald-950 hover:bg-emerald-50 transition"
                                  type="button"
                                >
                                  Open
                                </button>
                      ...
                      <div className="px-5 py-4 text-sm text-slate-700">
                        ...
                        <div className="mt-1 text-xs text-slate-600">Use “New session” to add one (optimistic rows appear instantly).</div>
                      </div>
                    )}
                  </div>
                </div>
      ...
      >
        <div className="rounded-2xl border border-slate-200/70 bg-white/60 p-4 text-sm text-slate-700">
          This drawer is UI-only for now. In enterprise systems, “Open” would update status → OPEN and activate live analytics.
          <div className="mt-3 text-xs text-slate-600">
            Selected session: <span className="font-mono text-slate-900">{selectedId ?? "—"}</span>
          </div>
        </div>
      ...
      >
        <div className="rounded-2xl border border-slate-200/70 bg-white/60 p-4 text-sm text-slate-700">
          This is a read-only edit drawer for enterprise feel (no backend writes).
          <div className="mt-3 text-xs text-slate-600">
            Selected session: <span className="font-mono text-slate-900">{selectedId ?? "—"}</span>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-slate-200/70 bg-slate-50/60 p-4">
          <div className="text-xs font-semibold tracking-widest text-slate-500">TEMPLATE APPLY (UI)</div>
          <div className="mt-2 text-sm font-semibold text-slate-900">Apply a template</div>
          <div className="mt-1 text-xs text-slate-600">
            This would prefill title + duration + suggested checklist preview (ready for wiring).
          </div>
