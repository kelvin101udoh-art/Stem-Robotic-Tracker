...
const UI_VERSION = "AI_ATTENDANCE_ENTERPRISE_V6_AUTOPILOT";

// If you wire Azure AI later, this page calls an API route:
// POST /api/ai/attendance-summary
// body: { sessionTitle, sessionTime, stats, notes, reasons }
// response: { engagement, integrity, improvement, skills, exportReady, coverage, punctuality }
// If that route doesn’t exist yet, we gracefully fall back to the rules-first summary.
...
export default function AttendancePage() {
    ...
    const stats = useMemo(() => {
        ...
        // Optional “reason code completeness” signals (nice for Copilot checklist)
        const missingLateReasons = entries.filter((x) => x.status === "late" && (x.late_reason ?? "").trim().length < 2)
            .length;
        const missingAbsentReasons = entries.filter(
            (x) => x.status === "absent" && (x.absent_reason ?? "").trim().length < 2
        ).length;

        return { present, late, absent, total, coverage, missingEvidence, missingLateReasons, missingAbsentReasons };
    }, [draft]);

    // ✅ NEW: AutoPilot – auto-focus the most important exception view
    useEffect(() => {
        if (!autoPilot) return;
        ...
        if (stats.missingEvidence > 0) setView("missing");
        else if (stats.late > 0) setView("late");
        else if (stats.absent > 0) setView("risk");
        else setView("all");
    }, [autoPilot, stats.missingEvidence, stats.late, stats.absent, students.length, selectedSessionId, isFinalised]);
    ...
    // ✅ NEW: “AI Assist Run” – fixes the most common issues in one click (still no auto-save)
    function runAIAssistPrep() {
        if (isFinalised) return;

        // Step 1: Focus missing evidence (AutoPilot already does, but we enforce)
        if (stats.missingEvidence > 0) setView("missing");

        // Step 2: Fill missing evidence for present/late only (on ALL learners, not just filtered view)
        setDraft((prev) => {
            const next = { ...prev };
            students.forEach((st) => {
                const row = next[st.id] ?? { status: "absent" as const, note: "", late_reason: "", absent_reason: "" };
        ...
        // Step 3: Generate a preview summary immediately
        setTimeout(() => {
            const s = buildSummaryFromDraft();
            setSummary({ ...s, source: "rules" });
            setSaveMsg("AI Assist completed (preview ready)");
    ...
    // ✅ NEW: live preview (even before Save)
    const liveSummary = useMemo(() => {
        if (!students.length || !selectedSessionId) return null;

        const notesCount = Object.values(draft).filter((d) => d.note.trim().length >= 6).length;
        if (notesCount < 2 && stats.coverage < 30) return null;

        return buildSummaryFromDraft();
    }, [draft, students.length, selectedSessionId, stats.coverage]);
    ...
    // ✅ NEW: Try Azure AI summary via API route, fallback to rules
    async function generateAzureSummaryPreview() {
        setAiError("");
        setAiBusy(true);

        try {
            const notes = students.map((st) => (draft[st.id]?.note ?? "").trim()).filter((n) => n.length >= 2);
            const sTitle = selectedSession?.title || "Session";
            const sTime = selectedSession?.starts_at ? `${formatDateTime(selectedSession.starts_at)}` : "—";

            const reasons = students.map((st) => {
            ...
            if (!res.ok) {
                throw new Error(`Azure AI route not available (${res.status})`);
            }
            ...
            if (!res.ok || isHtml) {
                throw new Error(
                    isHtml
                        ? "API route returned HTML (likely middleware redirect or missing route)"
                        : `API error ${res.status}: ${text.slice(0, 300)}`
                );
            }
            ...
            setSummary({ ...data, source: "azure" });
            setSaveMsg("Azure Copilot preview ready");
            setTimeout(() => setSaveMsg(""), 1400);
        } catch (e: any) {
            // fallback to rules
            const fallback = buildSummaryFromDraft();
            setSummary({ ...fallback, source: "rules" });
            setAiError("Azure AI not connected yet — using rules-first summary preview.");
        } finally {
            setAiBusy(false);
        }
    }
    ...
    async function saveRegister() {
        ...
        try {
            ...
            // After Save: generate summary (Azure if enabled, else rules)
            if (useAzureAI) {
                await generateAzureSummaryPreview();
            } else {
                const nextSummary = buildSummaryFromDraft();
                setSummary({ ...nextSummary, source: "rules" });
            }
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                                ...
                                className={cx(
                                    ...
                                    autoPilot ? "border-indigo-200 bg-indigo-50 text-indigo-900" : "border-slate-200 bg-white text-slate-700 hover:bg-indigo-50/60"
                                )}
                                title="Auto-focus exceptions-first workflow"
                            >
                            ...
                            <button
                                type="button"
                                onClick={() => setUseAzureAI((v) => !v)}
                                className={cx(
                                    "inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold",
                                    useAzureAI ? "border-cyan-200 bg-cyan-50 text-cyan-900" : "border-slate-200 bg-white text-slate-700 hover:bg-indigo-50/60"
                                )}
                                title="Use Azure AI for summary (requires API route)"
                            >
                                Azure Copilot: {useAzureAI ? "ON" : "OFF"}
                            </button>
                            ...
                            {isFinalised ? (
                                ...
                                <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-900 shadow-[0_0_0_3px_rgba(16,185,129,0.12)]">
                                    Synced
                                </span>
                            )}
                        </div>
            ...
            <div className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="max-w-3xl text-sm text-slate-600">
                        High-integrity registers + evidence capture with AutoPilot workflows and Azure Copilot-ready summaries.
                    </p>
                </div>
                    ...
                    <div className="lg:col-span-8">
                        ...
                        <div className="rounded-[22px] border border-slate-200 bg-gradient-to-b from-white via-white to-cyan-50/30 shadow-[0_16px_48px_-34px_rgba(2,6,23,0.35)]">
                            <div className="flex flex-col gap-4 border-b border-slate-200 bg-gradient-to-r from-transparent via-indigo-50/60 to-transparent px-5 py-4 sm:px-6">
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                            {todayBadge ? (
                                                <span className="inline-flex items-center rounded-full border border-slate-200 bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
                                                    Today
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center rounded-full border border-slate-200 bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
                                        ...
                                        <div className="mt-3 text-xs font-semibold tracking-widest text-slate-500">SESSION CONTEXT</div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            disabled={saving || !selectedSessionId || !students.length || isFinalised}
                                            className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 disabled:opacity-60"
                                        >
                                            Finalise
                                        </button>
                                    </div>
                            ...
                            <div className="grid gap-3 px-5 py-4 sm:grid-cols-3 sm:px-6">
                                <button
                                    ...
                                    className={cx(
                                        "rounded-2xl border bg-gradient-to-b from-white to-amber-50/50 p-4 text-left shadow-[0_10px_28px_-28px_rgba(2,6,23,0.25)] transition",
                                        view === "missing" ? "border-amber-500" : "border-slate-200 hover:bg-indigo-50/60"
                                    )}
                                >
                                ...
                                <button
                                    ...
                                    className={cx(
                                        "rounded-2xl border bg-gradient-to-b from-white to-cyan-50/50 p-4 text-left shadow-[0_10px_28px_-28px_rgba(2,6,23,0.25)] transition",
                                        view === "late" ? "border-cyan-500" : "border-slate-200 hover:bg-indigo-50/60"
                                    )}
                                >
                                ...
                                <button
                                    ...
                                    className={cx(
                                        "rounded-2xl border bg-gradient-to-b from-white to-rose-50/50 p-4 text-left shadow-[0_10px_28px_-28px_rgba(2,6,23,0.25)] transition",
                                        view === "risk" ? "border-rose-500" : "border-slate-200 hover:bg-indigo-50/60"
                                    )}
                                >
                            ...
                            <div className="grid gap-3 border-t border-slate-200 px-5 py-4 sm:grid-cols-4 sm:px-6">
                                ...
                                <KPI
                                    ...
                                    value={Object.values(draft).filter((x) => (x.note ?? "").trim().length >= 6).length}
                                    hint="Notes ≥ 6 chars"
                                />
                            </div>
                        </div>
                        ...
                        {!sessions.length ? (
                            ...
                            <>
                                    ...
                                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                        <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2">
                                                {(["all", "missing", "late", "risk"] as const).map((k) => (
                                                    ...
                                                    >
                                                        {k === "all" ? "All" : k === "missing" ? "Missing evidence" : k === "late" ? "Late" : "Absence risk"}
                                                    </button>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            <button
                                                ...
                                                className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-900 hover:bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 disabled:opacity-60"
                                            >
                                    ...
                                    <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-transparent via-indigo-50/60 to-transparent px-5 py-4 sm:px-6">
                                        ...
                                        <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
                                            ...
                                            <span className="text-slate-900">
                                                {view === "all" ? "All learners" : view === "missing" ? "Evidence exceptions" : view === "late" ? "Punctuality exceptions" : "Absence risk"}
                                            </span>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="grid gap-3 lg:grid-cols-12 lg:items-center">
                                                        ...
                                                        <div className="lg:col-span-3">
                                                            <div className="flex flex-wrap items-center gap-2">
                                                                ...
                                                                {/* Hover actions */}
                                                                <div className={cx("flex items-center gap-1 transition", "lg:opacity-0 lg:group-hover:opacity-100")}>
                                                        ...
                                                        <div className="lg:col-span-5">
                                                            ...
                                                            />
                                                            <div className="mt-1 text-xs text-slate-500">Tip: keep it skill-based and short for reporting.</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                ...
                                {/* AI session summary (official panel) */}
                                {summary ? (
                                    <div className="mt-4 rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_16px_48px_-34px_rgba(2,6,23,0.35)]">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div>
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <div className="text-xs font-semibold tracking-widest text-slate-500">AI SESSION SUMMARY</div>
                                                    <span
                                                        className={cx(
                                                    ...
                                                    >
                                                        {summary.source === "azure" ? "Azure Copilot" : "Rules-first"}
                                                    </span>
                                                    {aiError ? (
                                                ...
                                                <div className="mt-1 text-lg font-semibold text-slate-900">Export-ready insights for reporting</div>
                                                <div className="mt-1 text-sm text-slate-600">Generated from attendance + evidence notes (and Azure AI if enabled).</div>
                                            </div>
                        ...
                        <div className="rounded-[22px] border border-slate-200 bg-gradient-to-b from-white via-white to-cyan-50/30 shadow-[0_16px_48px_-34px_rgba(2,6,23,0.35)]">
                            <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-transparent via-indigo-50/60 to-transparent px-5 py-4 sm:px-6">
                                <div>
                                    ...
                                    <div className="mt-1 text-lg font-semibold text-slate-900">Copilot + AutoPilot operations</div>
                                </div>
                            ...
                            {copilotOpen ? (
                                <div className="space-y-3 px-5 py-4 sm:px-6">
                                    {/* ✅ NEW: Actionable Copilot controls */}
                                    <div className="rounded-2xl border border-slate-200 bg-white p-4">
                                        <div className="flex items-center justify-between">
                                            <div className="text-sm font-semibold text-slate-900">Copilot Actions</div>
                                        ...
                                        <div className="mt-3 grid gap-2">
                                            ...
                                            <button
                                                type="button"
                                                disabled={isFinalised || aiBusy}
                                                onClick={async () => {
                                                    if (!useAzureAI) {
                                                        const s = buildSummaryFromDraft();
                                                        setSummary({ ...s, source: "rules" });
                                                        setSaveMsg("Preview generated");
                                                        setTimeout(() => setSaveMsg(""), 1200);
                                                        return;
                                                    }
                                                    await generateAzureSummaryPreview();
                                                ...
                                                className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-900 hover:bg-indigo-50/60 disabled:opacity-60"
                                            >
                                                {aiBusy ? "Generating…" : useAzureAI ? "Generate Azure Copilot preview" : "Generate summary preview"}
                                            </button>
                                        </div>
                                    ...
                                    {/* ✅ NEW: Readiness checklist (so it never feels “silent”) */}
                                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                        <div className="text-[11px] font-semibold tracking-widest text-slate-500">READINESS</div>
                                        <div className="mt-2 space-y-1 text-sm">
                                            <div>✅ Register loaded</div>

                                            <div className={stats.missingEvidence ? "text-amber-900" : "text-slate-800"}>
                                                {stats.missingEvidence ? `⚠ Missing evidence: ${stats.missingEvidence}` : "✅ Evidence complete"}
                                            </div>

                                            <div className={stats.missingLateReasons ? "text-amber-900" : "text-slate-800"}>
                                                {stats.late
                                                    ? stats.missingLateReasons
                                                        ? `⚠ Late reasons missing: ${stats.missingLateReasons}`
                                                        : "✅ Late reasons complete"
                                                    : "✅ No late arrivals"}
                                            </div>

                                            <div className={stats.missingAbsentReasons ? "text-amber-900" : "text-slate-800"}>
                                                {stats.absent
                                                    ? stats.missingAbsentReasons
                                                        ? `⚠ Absence reasons missing: ${stats.missingAbsentReasons}`
                                                        : "✅ Absence reasons complete"
                                                    : "✅ No absences"}
                                            </div>

                                            <div className={stats.coverage < 80 ? "text-amber-900" : "text-emerald-900"}>
                                                {stats.coverage < 80 ? `⚠ Coverage low: ${stats.coverage}%` : `✅ Coverage strong: ${stats.coverage}%`}
                                            </div>

                                            <div className={isFinalised ? "text-emerald-900" : "text-slate-700"}>
                                                {isFinalised ? "✅ Finalised (locked)" : "⏳ Not finalised"}
                                            </div>
                                        </div>
                                    </div>

                                    {/* ✅ NEW: Live summary preview (appears earlier than Save) */}
                                    {liveSummary ? (
                                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                                            <div className="flex items-center justify-between">
                                                <div className="text-[11px] font-semibold tracking-widest text-slate-500">LIVE SUMMARY (PREVIEW)</div>
                                                <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-700">
                                                    Auto
                                                </span>
                                            </div>
                                        ...
                                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                                            ...
                                            <div className="mt-2 text-sm text-slate-600">
                                                Start marking attendance and add a couple of evidence notes to unlock preview insights.
                                            </div>
                                        </div>
                                    )}

                                    {/* Existing insight cards (still useful) */}
                                    <div className="rounded-2xl border border-slate-200 bg-white p-4">
                                        <div className="flex items-center justify-between">
                                        ...
                                        <div className="mt-1 text-sm text-slate-600">
                                            {stats.missingEvidence === 0
                                                ...
                                                : `Missing notes for ${stats.missingEvidence} marked learner(s). Use AI Assist to standardise evidence.`}
                                        </div>
                                    </div>
                                    ...
                                    <div className="rounded-2xl border border-slate-200 bg-white p-4">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="rounded-full border border-slate-200 bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-700">
                                                Coverage {stats.coverage}%
                                            </span>
                                        </div>
                                        <div className="mt-1 text-sm text-slate-600">
                                    ...
                                    <div className="rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 p-4">
                                        ...
                                        <div className="mt-1 text-sm text-slate-600">
                                            {isFinalised ? `Finalised at ${formatDateTime(finalisedAt)}` : "Finalise when complete to prevent post-hoc changes."}
                                        </div>
                                        {finalisedBy ? (
                                            <div className="mt-2 text-xs text-slate-500">Finalised by: {finalisedBy.slice(0, 8)}…</div>
                                        ) : null}
                                    </div>
                                </div>
                            ) : null}
