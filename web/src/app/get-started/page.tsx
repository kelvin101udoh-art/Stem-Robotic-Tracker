...
export default function GetStartedPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    // ✅ Validation
    if (!register.clubName.trim()) return openModal("error", "Error", "Please enter your club name.");
    if (!register.fullName.trim()) return openModal("error", "Error", "Please enter your full name.");

    if (!register.email.trim()) return openModal("error", "Error", "Please enter your email.");
    if (!isSchoolEmail(register.email)) {
      return openModal("error", "Error", "Please use a school/institution email address (no personal emails).");
    }

    if (register.password.trim().length < 6) {
      return openModal("error", "Error", "Password must be at least 6 characters.");
    }
    if (register.password !== register.confirmPassword) {
      return openModal("error", "Error", "Passwords do not match. Please confirm your password.");
    }
    ...
    try {
      ...
      // ✅ IMPORTANT FIX:
      // If email confirmation is enabled, provide emailRedirectTo so Supabase can build the confirmation link correctly.
      const emailRedirectTo =
        typeof window !== "undefined" ? `${window.location.origin}/get-started/confirm` : undefined;
      ...
      if (signUpError) {
        ...
        // Show this EXACT error as modal (as you requested)
        if (raw.includes("error sending confirmation email")) {
          openModal("error", "Error", "Error sending confirmation email.");
          throw signUpError;
        }
      ...
      // Try sign-in immediately (works only if confirmations are OFF or auto-confirm is enabled)
      const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({
        email,
        password,
      });

      // If confirmation is required, sign-in typically fails here.
      if (signInError || !signInData.user?.id) {
        openModal("success", "Update", "Account created. Please confirm your email, then return here to log in.");
      ...
      // Create club with unique generated code
      ...
      openModal("success", "Update", "Admin account created. Your club code is ready.");
    } catch (err: any) {
      // if we already showed a specific modal above, avoid double messaging
      const raw = (err?.message || "").toLowerCase();
      if (raw.includes("error sending confirmation email")) return;

      openModal("error", "Error", err?.message || "Registration failed. Please try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!login.email.trim() || !login.password.trim()) {
      return openModal("error", "Error", "Please enter your email and password.");
    }
    ...
    try {
      ...
      if (raw.includes("invalid login credentials")) {
        openModal("error", "Error", "Invalid login credentials.");
      } else {
        openModal("error", "Error", err?.message || "Login failed. Please try again.");
      }
    } finally {
      setLoading(false);
    }
