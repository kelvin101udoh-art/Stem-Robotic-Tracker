...
export default function AcceptInvitePage() {
  ...
  async function onAccept(e: React.FormEvent) {
    ...
    try {
      // 1) Create auth account using invite email (force email match)
      ...
      // 2) Try to sign in immediately (if email confirmation is OFF this will work)
      const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({
        email,
        password: password.trim(),
      });

      // If confirmation is required, stop here and tell user to confirm email.
      if (signInError || !signInData.user?.id) {
        setMsg("Account created. Please confirm your email, then return to this link to finish setup.");
        return;
      }

      const userId = signInData.user.id;

      // 3) Map invite role -> profiles.user_role
      const role: UserRole = invite.role; // teacher | student | parent

      // 4) Create/Upsert profile row
  ...
  const statusPill = (() => {
    if (loading) return { label: "Loading…", cls: "bg-slate-50 text-slate-600 border-slate-200" };
    if (error) return { label: "Action required", cls: "bg-rose-50 text-rose-700 border-rose-200" };
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:items-start">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm ring-1 ring-slate-200/40 sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Join your club workspace</h1>
                <p className="mt-2 text-sm text-slate-600">
                  Create your account to access your role dashboard. Your invite controls which role you receive.
                </p>
              </div>

              <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium ${statusPill.cls}`}>
                {statusPill.label}
              </span>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-2 grid gap-2 text-sm text-slate-700">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="font-medium">{invite?.role ? `${invite.role}` : "—"}</span>
                </div>
                <div className="flex items-center justify-between gap-3">
            ...
            <form onSubmit={onAccept} className="mt-6 space-y-4">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-slate-500">This must match the email the admin invited.</p>
              </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm ring-1 ring-slate-200/40 sm:p-8">
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              After you accept the invite, your account is linked to the club and routed to the correct dashboard.
              Club data stays separated by <span className="font-medium">club_id</span>.
            </p>
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Invites are single-use and expire automatically. After completion, the invite is marked as used.
              </p>
            </div>
          </div>
        </div>
